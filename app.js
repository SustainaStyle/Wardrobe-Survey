...
const els = {
  ...
  messageModalButton: document.getElementById("messageModalButton")
};
...
function showValidationMessage(message) {
  ...
  setTimeout(() => els.messageModalButton.focus(), 0);
}
...
els.messageModalButton.addEventListener("click", closeValidationMessage);

els.messageModal.addEventListener("click", event => {
  if (event.target.classList.contains("message-modal-backdrop")) {
    closeValidationMessage();
  }
});

document.addEventListener("keydown", event => {
  if (event.key === "Escape" && !els.messageModal.classList.contains("hidden")) {
    closeValidationMessage();
  }
});
...
function saveCurrentStep() {
  ...
  if (state.currentStep === 0) {
    ...
    if (!age || !identity || !occupation) {
      showValidationMessage(hi ? "कृपया सभी जानकारी भरें।" : "Please complete all three fields.");
      return false;
    }
  ...
  if (question.type === "checkbox") {
    ...
    if (!selected.length) {
      showValidationMessage(hi ? "कृपया कम से कम एक विकल्प चुनें।" : "Please select at least one option.");
      return false;
    }
  ...
  if (!selected) {
    showValidationMessage(hi ? "कृपया एक विकल्प चुनें।" : "Please select an option.");
    return false;
  }
